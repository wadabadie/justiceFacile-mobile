...
/// Dashboard for JURISTE / PSYCHOLOGUE / ONG. Focus on:
/// - certification status (blocking if not validated)
/// - available cases count
/// - active dossiers count
/// - shortcuts to planning / cas / mes dossiers
class HomeSpecialisteScreen extends StatefulWidget {
  const HomeSpecialisteScreen({super.key});

  @override
  State<HomeSpecialisteScreen> createState() => _HomeSpecialisteScreenState();
}

class _HomeSpecialisteScreenState extends State<HomeSpecialisteScreen> {
  UserEntity? _user;
  String _certifStatut = 'NON_SOUMIS';
  int _casDispoCount = 0;
  int _mesDossiersCount = 0;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    // Only fetch counters if certified — otherwise backend returns [] anyway.
    if (_certifStatut == 'VALIDE') {
      ...
      try {
        ...
        _mesDossiersCount = (res.data as List).length;
      } on DioException catch (_) {}
    }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(child: _StatCard(
                          ...
                          value: _loading ? '…' : '$_mesDossiersCount',
                    ...
                    _ActionRow(
                      ...
                      label: s.homeSpecPlanningLabel,
                      desc: s.homeSpecPlanningDesc,
                    ...
                    _ActionRow(
                      ...
                      label: s.homeSpecMessagesLabel,
                      desc: s.homeSpecMessagesDesc,
                      color: AppColors.bleuMid,
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          u != null ? '${s.homeGreeting}, ${u.firstName}' : s.homeGreeting,
                          style: const TextStyle(
                            fontFamily: 'GoogleSans', fontSize: 15,
                            color: Color(0xB3FFFFFF),
                          ),
                        ),
                        Text(u != null ? _roleLabel(u.role) : '',
                            style: const TextStyle(
                              fontFamily: 'GoogleSans', fontSize: 18,
                              fontWeight: FontWeight.w700, color: AppColors.orPale,
                            )),
  ...
  ({Color bg, Color fg, IconData icon, String title, String desc, String cta}) _cfg() {
    return switch (statut) {
      'EN_ATTENTE' => (
        ...
        title: s.certStatutEnAttenteTitle,
        desc: s.certStatutEnAttenteDesc,
        cta: s.btnConsult,
      ),
      'REJETE' => (
        bg: AppColors.rougeLight, fg: AppColors.rouge,
        icon: Icons.cancel_outlined,
        title: s.certStatutRejeteTitle,
        desc: s.certStatutRejeteDesc,
        cta: s.homeSpecCertifCta,
      ),
      _ => (
        bg: AppColors.orLight, fg: AppColors.orDark,
        icon: Icons.workspace_premium_outlined,
        title: s.certStatutNonSoumisTitle,
        desc: s.certStatutNonSoumisDesc,
        cta: s.homeSpecCertifCta,
      ),
    };
  }
